...
class Report extends CI_Controller {
    ...
    public function return()
    {
        ...
        if(empty($tgl_awal) or empty($tgl_akhir))
        { // Cek jika tgl_awal atau tgl_akhir kosong, maka :
            $transaksi = $this->M_report->view_all_return();  // Panggil fungsi view_all yang ada di M_report
    ...
    public function pdf_return()
    {
        $tgl_awal = $this->input->get('tgl_awal'); // Ambil data tgl_awal sesuai input (kalau tidak ada set kosong)
        $tgl_akhir = $this->input->get('tgl_akhir'); // Ambil data tgl_awal sesuai input (kalau tidak ada set kosong)
        if(empty($tgl_awal) or empty($tgl_akhir)){ // Cek jika tgl_awal atau tgl_akhir kosong, maka :
            $data_req = $this->M_report->view_all_return();  // Panggil fungsi view_all yang ada di M_report
            $label = 'All Data Request';
        }else{ // Jika terisi
            $data_req = $this->M_report->view_by_date_return($tgl_awal, $tgl_akhir);  // Panggil fungsi view_by_date yang ada di M_report
            $tgl_awal = date('d-m-Y', strtotime($tgl_awal)); // Ubah format tanggal jadi dd-mm-yyyy
            $tgl_akhir = date('d-m-Y', strtotime($tgl_akhir)); // Ubah format tanggal jadi dd-mm-yyyy
            $label = 'Periode Date '.$tgl_awal.' till '.$tgl_akhir;
        }
        ...
        $data['data_req'] = $data_req;
        $data['period'] = $tgl_awal . ' till ' . $tgl_akhir;
        $data['title'] = 'Report Return';

		$data['siswa'] = "tes";
		$this->load->library('pdf');
		$this->pdf->setPaper('A4', 'potrait');
		$this->pdf->filename = "laporan-data-siswa.pdf";
		$this->pdf->load_view('admin/v_report/print_return', $data);
